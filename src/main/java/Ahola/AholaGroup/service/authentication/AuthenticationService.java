...
//      send email alert
    ...
    public AppResponse<String> resetPassword(AuthenticationRequest authenticationRequest){
        Admin userEmail = adminRepo.findByEmail(authenticationRequest.getEmail()).orElseThrow(()-> new ApiException("email not Existing"));
        userEmail.setPassword(passwordEncoder.encode(authenticationRequest.getPassword()));
        adminRepo.save(userEmail);
        return new AppResponse<>(0, "password has been successfully saved");
    }
